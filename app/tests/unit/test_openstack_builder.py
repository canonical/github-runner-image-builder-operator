...
function enable_armhf_multiarch() {{
    ...
    # architecture normally served from ports.ubuntu.com. That official ports mirror is not
    # reachable on every build network (returns 403 Forbidden), while the image's own configured
    # mirror already carries ports architectures (it serves this VM's own native arm64 packages),
    # so reuse it for armhf instead of hardcoding ports.ubuntu.com.
    native_mirror=$(/usr/bin/awk '/^URIs:/ {{print $2; exit}}' /etc/apt/sources.list.d/ubuntu.sources)
